...
0.7.0 按用户提供的分栏参考精简界面：顶部搜索与类型筛选、左侧单行日期列表、右侧默认预览，减少常驻说明。浅色保持克制清晰，次要操作集中在底部图标与菜单。本轮本机隔离回归、布局检查与三端构建通过，本机已更新，公开预览与四安装包已交付；详细结果和实机验收状态见 [浅色极简界面验收](docs/浅色极简界面验收.md)。
...
2. 按全局快捷键，搜索关键词，或通过右上角类型选择切换「全部类型 / 文本 / 图片 / 收藏」。
3. 用 **↑ / ↓** 选择，按 **Enter** 粘贴。

| 操作 | 方法 |
|---|---|
| 原格式 / 纯文本 | 底部「原格式 / 纯文本」，同时影响「仅复制」和「粘贴」 |
| 仅复制 | Ctrl+Enter，或底部复制图标 |
| 关闭面板 | Esc；不改变剪贴板 |
| 收藏 / 取消收藏 | 点击记录右侧星标，或 Ctrl+D |
| 文本、图片预览 | 新布局默认打开；Ctrl+Space 或底部预览图标开关，宽窗口并排、窄窗口上下排列 |
| 独立完整预览 | 右键记录 → 独立预览窗口；较大内容的侧栏也提供完整查看按钮 |
| 修改收藏名称、删除 | 右键记录；Ctrl+Delete 删除当前记录 |
| 暂停、忽略下一次复制 | 底部「···」 |
| 快捷键、外观、自启动 | 底部「···」→ 设置 → 常规 |
| 保存规则、容量与单条限制 | 设置 → 历史与存储；条数和单条上限在「更多限制」中 |
| 更改历史保存文件夹 | 设置 → 历史与存储 → 保存位置，或从「空间与内存」进入 |
| 存储与隐私说明、清空历史、退出 | 底部「···」菜单 |
...
设置仍分为「常规」「历史与存储」「空间与内存」，数据和清理选项保持不变。
