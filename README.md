...
| 文本、图片预览 | Ctrl+Space，或「预览」；宽窗口并排显示，窄窗口上下排列。图片支持原始尺寸 |
...
预览关闭时不加载完整正文；关闭或隐藏主面板时释放当前预览。文本超过 1 MiB、图片超过 16 MiB 时，内嵌区域先显示摘要，点击「查看完整内容」或右键打开完整预览。保存和复制的数据保持完整；明确打开大内容仍可能需要等待。

![深色图片预览，使用合成图像](docs/images/raycast-preview-dark.png)

![分组设置，使用合成数据](docs/images/raycast-settings.png)
